...
class TierValidationFundMixin(models.AbstractModel):
    ...
    def _promote_created_reviews(self, reviews):
        """Promover las revisiones recién creadas del contexto activo.

        Ver `tier.review._fund_promote_to_pending`.
        """
        for rec in self:
            rec_reviews = reviews.filtered(
                lambda r, rec=rec: r.model == rec._name and r.res_id == rec.id
            )
            rec_reviews._fund_promote_to_pending(rec._current_context_reviews())
